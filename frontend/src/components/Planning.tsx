...
export const Planning: React.FC<Props> = ({ currentUser }) => {
    ...
    const handleCreateLeave = async (e: React.FormEvent) => {
        e.preventDefault();
        // days_count is computed server-side from the dates — no need to send it.
    ...
    const handleValidation = async (leaveId: number, status: 'APPROVED' | 'REJECTED') => {
        const res = await api.put(`/api/leaves/${leaveId}/status`, { status });
        if (res.ok) fetchLeaves();
    };

    const handleSaveEditLeave = async (leaveId: number) => {
        // days_count is recomputed server-side from date_start/date_end — no need to send it.
        const res = await api.put(`/api/leaves/${leaveId}`, {
    ...
    return (
        <div className="space-y-8 animate-fade-in pb-12">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex bg-slate-50 p-1 rounded-lg overflow-x-auto no-scrollbar">
                    <button onClick={() => setActiveTab('GLOBAL')} className={`shrink-0 px-4 py-2 rounded-md text-sm font-bold transition-all ${activeTab === 'GLOBAL' ? 'bg-slate-100 text-slate-900 shadow' : 'text-slate-500 hover:text-slate-900'}`}>PLANNING</button>
                    <button onClick={() => setActiveTab('MY_LEAVES')} className={`shrink-0 px-4 py-2 rounded-md text-sm font-bold transition-all ${activeTab === 'MY_LEAVES' ? 'bg-slate-100 text-slate-900 shadow' : 'text-slate-500 hover:text-slate-900'}`}>MES CONGÉS</button>
                    {currentUser.role === 'admin' && (
                        <button onClick={() => setActiveTab('VALIDATION')} className={`shrink-0 px-4 py-2 rounded-md text-sm font-bold transition-all ${activeTab === 'VALIDATION' ? 'bg-slate-100 text-slate-900 shadow' : 'text-slate-500 hover:text-slate-900'}`}>GESTION DES CONGÉS</button>
                    )}
                </div>
            </div>
